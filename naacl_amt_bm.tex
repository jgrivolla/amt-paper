...
As explained in Section \ref{sect:design}, a total amount of 9000 assignments were uploaded on AMT. At a reward of .02\$ per assignment, a total sum of 225\$ (180\$ + 45\$ Amazon fees) was spent on the task. Workers perceived an average hourly rate of 6.5\$/hour for HIT1 and 8\$/hour for HIT2 and HIT3. These figures suggest that 



% 6.5 $/hora -> 8 $/hora

To compute the in-House costs, the expert or engineer is paid at a rate of 70\$ an hour. The in-house annotation tasks of 1000 sentences would last for three hours, giving a cost of 210\$ and a marginal cost of preparing the data. 


 as is suggested in ... \texttt{<This is another useful reference: \cite{mason_financial_2009}>}

As this task can become tiresome and hard it needs to be elapsed in time. In AMT the a hit may be distributed into many volunteers that 24 hours a day are ready to work in parallel to produce the results in a few hours.     
...
% In these experiments reported in this paper, we have relied exclusively on To date, We have used only AMT annotations from HIT1 to perform
% Further work is needed to fully analyze the impact of each of the three HIT designs
